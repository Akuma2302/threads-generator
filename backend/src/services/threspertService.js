...
/**
 * Calls the Threspert agent (currently backed by a free model on OpenRouter,
 * e.g. google/gemma-4-31b-it:free) to generate the post content based on
...
async function generateThreadContent(input) {
  ...
  for (let i = 0; i < modelsToTry.length; i++) {
    ...
    try {
      ...
      if (i > 0) {
        console.warn(`[threspert] Primary model failed, succeeded on fallback: ${model}`);
      }

      return { variations };
    } catch (err) {
      const providerMessage = err.response?.data?.error?.message || err.message;
      console.error(`[threspert] Model "${model}" failed: ${providerMessage}`);
  ...
  const error = new Error(
    `Threspert couldn't generate a post right now — every configured model failed (${attemptErrors.join(
      '; '
